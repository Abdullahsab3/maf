    ...
    def onBenchmark(benchmark: String): Unit =
        ...
        property(s"The Scheme interpreters give the same result for $benchmark", InterpreterTest, SlowTest) {
            try
                ...
                val r1: ConcreteValues.Value =
                    try interpreter.run(program, Timeout.start(Duration(2, MINUTES)))
                    catch case e: ProgramError => ConcreteValues.Value.Error(e)

                val r2: ConcreteValues.Value =
                    try CPSinterpreter.run(program, Timeout.start(Duration(2, MINUTES)))
                    catch case e: ProgramError => ConcreteValues.Value.Error(e)
