...
object  Benchmark extends App:
    ...
    if(args.length > 1) then
        ...
        if(args.length > 3) then 
            val latticeImplementation = args(2)
            if(latticeImplementation == "-ll") then 
                llAnalysis = true 
    ...
    def newCPStandardAnalysis(program: SchemeExp) =
        new SimpleSchemeModFAnalysis(program)
            with SchemeModFNoSensitivity
    ...
    while(i < rounds) do
        ...
        while(j < bench.length) do
            runAnalysis(j ,program => if(llAnalysis) then newLLStandardAnalysis(program) else newCPStandardAnalysis(program), () => Timeout.start(Duration(1, MINUTES)))
            j = j + 1
        i = i + 1
    ...
    writer.write(s"${if(llAnalysis) then "LLNativeDomain" else "CPNativeDomain"}\n")
