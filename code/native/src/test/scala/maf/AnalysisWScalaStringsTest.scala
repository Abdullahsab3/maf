...
class AnalysisWScalaStringsTest extends AnyFlatSpec with should.Matchers:
    ...
    def newNativeAnalysisWithScalaStrings(program: SchemeExp) =
        new SimpleSchemeModFAnalysis(program)
            with SchemeModFNoSensitivity
                ...
                store.foreach((addr, v) =>
                    v.contents.foreach(
                        (key, el) =>
                            el match
                                case str: modularLattice.Str =>
                                    nativeStrings += (addr.asInstanceOf[Address], str.s.asInstanceOf[String])
                                case str: modularLattice.Symbol =>
                                    nativeSyms += (addr.asInstanceOf[Address], str.s.asInstanceOf[String])
                                case int: modularLattice.Int =>
    ...
    "Native Analysis using Scala strings" should "return correct and sound results" in {
        testFiles.foreach(file =>

            val text = SchemeParser.parseProgram(Reader.loadFile(file))
            val cp = newCPAnalysis(text)
            val sn = newNativeAnalysisWithScalaStrings(text)
