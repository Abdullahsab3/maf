...
class AnalysisWoGCTest extends AnyFlatSpec with should.Matchers:
    ...
    def newNativeAnalysisWithoutGC(program: SchemeExp) =
        new SimpleSchemeModFAnalysis(program)
            with SchemeModFNoSensitivity
    ...
    "Native Analysis without Garbage Collection" should "return correct and sound results" in {
        testFiles.foreach(file =>

            val text = SchemeParser.parseProgram(Reader.loadFile(file))
            val cp = newCPAnalysis(text)
            val sn = newNativeAnalysisWithoutGC(text)
